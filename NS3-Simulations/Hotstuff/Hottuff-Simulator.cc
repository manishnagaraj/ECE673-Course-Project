 ...
 * using NS3
 *
 * Hotstuff Algorithm - Simulator Code
 * Needs to be linked with application
 * With the help of Hotstuff-helper and Hotstuff 
 *
...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("Hotstuff-Simulator");
...
int main(int argc, char *argv[])
{
	...
	if(verbose)
	{
		LogComponentEnable("Hotstuff", LOG_LEVEL_INFO);
		LogComponentEnable("HotstuffHelper", LOG_LEVEL_INFO);
		LogComponentEnable("Hotstuff-Simulator", LOG_LEVEL_INFO);
	}
	...
	HotstuffHelper Hotstuff ("ns3::UdpSocketFactory", 
		              Address (InetSocketAddress (Ipv4Address::GetAny (), port)), num_faults);
	 
	ApplicationContainer app = Hotstuff.Install (nodes);
