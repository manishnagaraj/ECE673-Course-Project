 ...
 * using NS3
 *
 * DolevStrong Algorithm - Simulator Code
 * Needs to be linked with application
 * With the help of DolevStrong-helper and DolevStrong 
 *
...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("DolevStrong-Simulator");
...
int main(int argc, char *argv[])
{
	...
	if(verbose)
	{
		LogComponentEnable("DolevStrong", LOG_LEVEL_INFO);
		LogComponentEnable("DolevStrongHelper", LOG_LEVEL_INFO);
		LogComponentEnable("DolevStrong-Simulator", LOG_LEVEL_INFO);
	}
	...
	DolevStrongHelper DolevStrong ("ns3::UdpSocketFactory", 
		              Address (InetSocketAddress (Ipv4Address::GetAny (), port)), num_faults);
	 
	ApplicationContainer app = DolevStrong.Install (nodes);
